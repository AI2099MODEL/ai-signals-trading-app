...
object IndianCommodityRepository {
    ...
    suspend fun fetchCommodityData(symbolKey: String, skipWsCache: Boolean = false): CommodityQuote? = withContext(Dispatchers.IO) {
        ...
        // 1. Try Dhan API
        try {
            Log.d("IndianCommodityService", "Trying Dhan API for $baseSymbol")
            val dhanResponse = dhanRetrofit.getCommodityQuote(baseSymbol.lowercase())
            if (dhanResponse != null) {
                Log.d("IndianCommodityService", "Dhan API response received for $baseSymbol: $dhanResponse")
                ...
                Log.d("IndianCommodityService", "Dhan API returned null for $baseSymbol")
            }
        } catch (e: Exception) {
            Log.d("IndianCommodityService", "Dhan REST quote feed unavailable for $baseSymbol, using exchange feed: ${e.message}")
        }
        ...
        try {
            Log.d("IndianCommodityService", "Trying Yahoo Finance for $baseSymbol")
