...
object DhanPortfolioService {
    ...
    suspend fun fetchDhanPortfolio(context: Context): DhanFetchResult = withContext(Dispatchers.IO) {
        ...
        if (apiSuccess) {
            return@withContext DhanFetchResult(
                success = true,
                holdings = emptyList(),
                message = "No active holdings or positions returned from Dhan HQ account.",
                isDemoData = false
            )
        }

        val isCredsMissing = accessToken.isBlank() || accessToken == "MY_DHAN_ACCESS_TOKEN" || accessToken == "YOUR_ACCESS_TOKEN" || accessToken == "YOUR_CLIENT_ID"
        val msg = if (isCredsMissing) {
            "Dhan API credentials not configured. Please enter your Client ID & Access Token in Settings to sync live portfolio."
        } else {
            statusMessage.ifBlank { "Failed to connect to Dhan HQ API. Please verify your credentials in Settings." }
        }
    ...
    suspend fun getQuote(context: Context, ticker: String): Double? = withContext(Dispatchers.IO) {
        val (clientId, accessToken, apiKey) = getDhanCredentials(context)
        val cleanTicker = ticker.replace("-EQ", "").uppercase().trim()
        
        // Dhan quote endpoint: /v2/quotes?symbols=SYMBOL
        val jsonStr = makeDhanApiRequest("/v2/quotes?symbols=$cleanTicker", clientId, accessToken, apiKey)
        if (!jsonStr.isNullOrBlank()) {
            try {
                val obj = JSONObject(jsonStr)
                if (obj.has("data")) {
                    val data = obj.getJSONObject("data")
                    if (data.has(cleanTicker)) {
                        return@withContext data.getJSONObject(cleanTicker).optDouble("ltp", 0.0).takeIf { it > 0.0 }
                    }
                }
            } catch (e: Exception) {
                Log.e(TAG, "Error parsing Dhan quote: ${e.message}")
            }
        }
